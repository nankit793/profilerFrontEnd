  ...
  return (
    <>
      <div className="w-[100%] h-full  text-center px-2">
        ...
        <div className="">
          {usersArray && usersArray.length >= 1 &&  usersArray.map((user, index)=>{ 
            return <div onClick={
              ()=>{
                router.push(`/home/${user.id}`);
              }
            } className="w-full bg-color_3 cursor-pointer flex justify-start gap-2 align-center py-2 hover:bg-color_1 px-2 border rounded-lg  my-2">
              <div className="  w-[40%]">
                <div className="rounded-full w-[50px] h-[50px] bg-color_2">
                  
                </div>
              </div>
              <div className="text-left flex flex-col justify-between w-full"> 
               <div className=" font-semibold text-color_2">{user.name}</div> 
